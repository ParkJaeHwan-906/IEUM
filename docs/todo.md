...
- [~] Service / Controller 계층 — 뼈대 생성 (2026-09-12). `ieum-api` / `orders/` 아래 `config`·`stock`·`exception`·`service`·`web`
  ...
  - [x] **2단계 낙관적 락 + 재시도 계층** (2026-09-17 라운드 2 와 (선택) 7 까지 완료) — 구현 순서와 함정은 [2단계 가이드](./guides/stage2-optimistic-retry-guide.md)
    ...
    - [x] 6. 측정 (라운드 1 09-14, 라운드 2 09-17) — `STOCK_STRATEGY=optimistic` 으로 같은 k6 시나리오. 기록할 것: 201 이 정확히 100 인지, 최종 `remaining_quantity`, 소진(exhausted) 건수, 시도 횟수 분포, `hikaricp.connections.pending` 최대, p99, 처리량
      ...
      - [x] 라운드 2 (2026-09-17) — 201 정확히 100, **500 0, `deadlock` 0, 롤백된 INSERT 0**. 충돌 1,894, 503 574 (5.7%) 가 "재고가 있는데 답을 못 준 요청" 의 진짜 값. p99 790ms, ≈ 719 req/s. 같은 문서에 이어서 기록, ADR-0003 표·결정 갱신
